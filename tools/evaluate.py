...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl, n_classes):
        ## evaluate
        hist = torch.zeros(n_classes, n_classes).cuda().detach()#.detach()代表不参与网络更新
        ...
        for i, (imgs, label) in diter:
            # print(label.shape)
            N, _, H, W = label.shape# 2 1 1024 2048
            label = label.squeeze(1).cuda()#2 1024 2048
            size = label.size()[-2:]#torch.size([1024,2048])
            probs = torch.zeros(
                    (N, n_classes, H, W), dtype=torch.float32).cuda().detach()#[2,2,1024,2048]
            for scale in self.scales:
                sH, sW = int(scale * H), int(scale * W)#1024,2048
...
#######################################################################################################
            ...
            preds = torch.argmax(probs, dim=1)#[2,2,1024,2048]
            keep = label != self.ignore_label#label：[2,1024,2048]
...
def eval_model(net, ims_per_gpu, im_root, im_anns):
    ...
    if dist.is_initialized() and dist.get_rank() != 0:
        ...
        diter = enumerate(tqdm(dl))  # caiyancheng写的测速度
    for i, (imgs, label) in diter:
        imgs = imgs.cuda()
        # imgs = F.interpolate(imgs, size=(256, 512),mode='bilinear', align_corners=True)  # 上/下采样
        begin = time.time()
        logits=net(imgs)
        after=time.time()
        logits=logits
